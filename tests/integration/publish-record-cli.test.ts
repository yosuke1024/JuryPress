...
import { buildInitialRecord, readRecord, recordsDir, contentHash } from '../../src/lib/generation/record-store';
...
describe('publish-record CLI — per-record commit verification', () => {
  ...
  /** Where a published review landed, read from the record the CLI itself wrote. */
  function publishedReviewDir(recordId: string, slug: string): string {
    const publishedAt = readRecord(contentRoot, recordId)?.publication.publishedAt;
    if (!publishedAt) {
      throw new Error(`publishedReviewDir() needs a published record; ${recordId} has no publishedAt.`);
    }
  ...
  it('publishes the target even though HEAD moved on for an unrelated commit', () => {
    ...
    // Derived from the record's own publishedAt, not from a hardcoded month: a review is filed
    // under the month it was published in, so a literal date made this pass only during the
    // month it was written in. See reviewDir() in editorial-flow.test.ts.
    expect(fs.existsSync(path.join(publishedReviewDir(idA, slugA), 'review.json'))).toBe(true);
  });
