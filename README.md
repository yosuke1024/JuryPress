...
- `PUBLIC_ADMAX_ENABLED` / `PUBLIC_ADMAX_ARTICLE_{1,2}_{PC,SP}`: (Optional) 忍者AdMax, used when no AdSense client is injected — then no Google ad code loads at all. Frames are fixed-size (300x250) and split by device category, so each placement takes a PC frame and a smartphone frame. `src/lib/ads.ts` owns the choice and `tests/unit/ads.test.ts` pins it.
